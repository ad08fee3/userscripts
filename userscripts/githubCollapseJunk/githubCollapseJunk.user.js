    ...
    const classifiers = [
        { tier: 1, name: 'Deleted file', displayName: 'Deleted', classify: (fileMeta) => fileMeta.status === 'REMOVED' },
        {
            tier: 1, name: 'Generated file', displayName: 'Auto-generated', classify: (fileMeta) => {
                // `.gen.` / `_gen.` before the extension is a widespread
                // generated-file naming convention (osis_config.gen.json,
                // oas_client_gen.go), and it's the only signal for formats
                // like JSON that can't carry a header comment.
                return fileMeta.newTreeEntry?.isGenerated === true || /[._]gen\.[a-z0-9]+$/i.test(fileMeta.path);
            }
        },
        ...
        {
            // Generic "GENERATED by <tool> - do not edit" banner in the first few
            // lines. Covers hand-rolled generators that don't emit one of the
            // tool-specific headers above.
            tier: 3,
            name: 'Generated header',
            displayName: 'Auto-generated',
            shouldFetch: (fileMeta) => /\.(yaml)$/.test(fileMeta.path),
            classify: (fileMeta, fileContent) => /^(?:[#/*;\-\s]*)(?:auto[-\s]?)?(?:code\s+)?generated by\b[^\n]*\bdo not edit\b/im.test(
                fileContent.split('\n').slice(0, 5).join('\n')
            ),
        },
